...
useChatCommand(builder, async (interaction: ChatInputCommandInteraction) => {
    ...
    if (channel) {
        channel.send({
            content: "**Please read the information above before applying for Santa Squad.**",
...
useButton("santa:apply", async (interaction: ButtonInteraction) => {
    const apps = await useClient().client.channels.fetch(CHANNELS.MAIN.santa_applications);
    if (!apps?.isTextBased()) {
...
useButton("santa:accept", async (interaction: ButtonInteraction, [outcome, userId]) => {
