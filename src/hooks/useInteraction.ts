    ...
    useEvent("interactionCreate", async (interaction: Interaction) => {
        ...
        try {
            ...
            if (response instanceof ModalBuilder) {
                if (alreadyReplied) {
                    throw new Error(`Cannot show a modal to an already replied or deferred interaction`);
                }
                if (!("showModal" in interaction)) {
                    throw new Error(`Cannot show a modal to interaction of type ${interaction.type}`);
                }
                await interaction.showModal(response);
            } else {
                await (alreadyReplied ? interaction.editReply(response) : interaction.reply(response));
            }
        } catch (error) {
