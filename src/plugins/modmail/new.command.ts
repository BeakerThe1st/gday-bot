...
const builder = new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
        option.setName("user").setDescription("User to create a thread for").setRequired(true)
    )
...
useChatCommand(builder, async (interaction: ChatInputCommandInteraction) => {
    ...
    const newThread = await MailThread.create({author: target.id, initialMessage: ""});
    const notificationEmbed = new EmbedBuilder()
        .setTitle("Thread Created")
        .setDescription("The r/Apple mod team have created a modmail thread with you. Any messages sent here will be forwarded to the team.")
